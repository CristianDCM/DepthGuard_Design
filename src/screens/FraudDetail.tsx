...
export default function FraudDetail() {
  ...
  return (
    <div className="min-h-screen pb-24 flex flex-col bg-dg-bg">
      <header className="sticky top-0 z-50 bg-dg-bg flex items-center justify-between px-6 py-4">
      ...
      <main className="max-w-2xl mx-auto px-6 pt-4 pb-32 space-y-8 w-full">
        ...
        <section className="space-y-6">
          <h2 className="font-headline font-bold text-xl text-white px-1">Análisis Biométrico</h2>
          <div className="space-y-5 bg-dg-card p-6 rounded-3xl border border-dg-border">
            <MetricRow label="Confianza" value="N/A" progress={0} />
            <MetricRow label="Varianza de Profundidad" value="0.3" progress={15} color="bg-dg-error" />
            <MetricRow label="Rango 3D" value="0.8 cm" progress={10} color="bg-dg-error" />
            <MetricRow label="Distancia" value="70 cm" progress={60} color="bg-blue-500" />
          </div>
        </section>

        <div className="flex flex-col gap-3">
          <button className="btn-primary w-full h-14 flex items-center justify-center gap-2">
            <FileText className="w-5 h-5" /> Descargar Informe (PDF)
          </button>
          <button 
            onClick={() => navigate(-1)}
...
function MetricRow({ label, value, progress, color = "bg-dg-accent" }: { label: string, value: string, progress: number, color?: string }) {
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-end">
        <span className="text-dg-text-muted text-sm font-medium">{label}</span>
        <span className="text-white font-headline font-bold">{value}</span>
      </div>
      <div className="h-2 w-full bg-dg-bg rounded-full overflow-hidden">
        <motion.div 
          initial={{ width: 0 }}
